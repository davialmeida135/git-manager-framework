...
@Service("dateSortingStrategy")
public class DateSortingStrategy implements TaskSortingStrategy {
    ...
    public List<Tarefa> sort(List<Tarefa> tasks) {
        ...
        List<Tarefa> sortedSchedule = tasks.stream()
                .filter(s -> LocalDate.parse(s.getPrazo(), formatter).compareTo(today) >= 0) // Filter tasks with
                                                                                             // deadlines today or in
                                                                                             // the future
